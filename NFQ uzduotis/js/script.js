...
$('.buy_me_button').on('click', function (){
  $('.guitar_div').css({'transform':'scale(0.7, 0.7) rotate(-90deg)', //animates guitar image
                  'margin-right':'60%',
                  'margin-top':'20%'
                });
  $('.homemade_text').css({'opacity': '0', 'left': '30%'}, function (){ //animates text from homepage
      $(this).hide();
    });
  $('.buy_me_button').css({'transform':'scale(3, 3)', 'opacity':'0'}, function (){ //animates button from homepage
      $(this).hide();
  });
...
$('.buy_button').on('click', function(){
  ...
  if(validate(buyer)){ // this expression validates if buyer filled-up everything correctly (validate_form.js)
    $('.order_div').css({'transition':'all ease 1s', 'left': '0', 'opacity':'0'});
    setTimeout(function() {
        $('.order_div').hide();
    }, 1000);
    $('.shipping_info_div').show().css({'transition': 'all ease 1s', 'z-index':'3', 'left': '43%', 'opacity':'1'}); //shows shipping information to check
    var i = 0;
...
$('.incorrect').on('click', function () {
  ...
  $('.order_div').show().css({'z-index':'3', 'transition':'all ease 1s', 'left': '43%', 'opacity':'1'});
});

// buyer confirms the informtion is correct
$('.correct').on('click', function () {
  hideShippingInfo (); //functions.js
  $('.confirmation_div').show().css({'z-index':'3', 'transition':'all ease 1s', 'left': '43%', 'opacity':'1'});
